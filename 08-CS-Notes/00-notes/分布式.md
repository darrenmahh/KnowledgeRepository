...
> [!tip] ACID 特性
>
> **ACID** 是数据库事务管理中的四个关键特性，确保数据操作的可靠性与一致性。
>
> - **A: Atomicity（原子性）**  
>   事务是一个不可分割的单位，要么全部执行成功，要么全部失败回滚。  
>   *示例：银行转账中，扣款和加款必须同时完成或都不完成。*
>
> - **C: Consistency（一致性）**  
>   事务执行前后，数据库必须保持一致性状态，满足所有约束和规则。  
>   *示例：账户余额不能为负数。*
>
> - **I: Isolation（隔离性）**  
>   多个事务并发执行时，彼此之间不应互相干扰。  
>   *常见级别：读未提交、读已提交、可重复读、串行化。*
>
> - **D: Durability（持久性）**  
>   一旦事务提交成功，其对数据库的更改是永久性的，即使系统崩溃也不会丢失。  
>   *示例：交易记录写入磁盘后不会因断电而丢失。*
>
> > [!SUMMARY]
> > **一句话总结：**  
> > ACID 确保事务在并发和异常情况下都能正确执行，保障数据的完整性和一致性。
...
> [!tip] 例
> 中行转建行--跨行转账服务：
> 协调者让中行扣钱 让建行加钱  然后问两者是否全部可以实现，可以的话，就同时提交，不然就都不提交
